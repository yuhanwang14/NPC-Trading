...
namespace npcTrading
{
  ...
  class BinanceExecutionClientImpl
  {
   public:
          ...
          running_(false)
    {
      ...
      {
        config_.rest_base_url = config_.use_testnet ? "https://testnet.binance.vision" : "https://api.binance.com";
      }
      if (config_.ws_base_url.empty())
      {
        config_.ws_base_url = config_.use_testnet ? "testnet.binance.vision" : "stream.binance.com";
      }

      // Initialize Rate Limiter: 1200 request weight per minute (Binance standard)
      // Burst size set to 100 for reasonable burst capacity
      rate_limiter_ = std::make_unique<utils::TokenBucketRateLimiter>(1200.0, 100.0);
    }
    ...
    void connect()
    {
      ...
      ws_thread_ = std::thread([this]() { run_user_data_stream(); });

      // Start keepalive thread
      keepalive_thread_ = std::thread([this]() { run_keepalive_loop(); });

      connected_ = true;
      std::cout << "[BinanceExecutionClient] Connected (testnet=" << (config_.use_testnet ? "true" : "false") << ")"
                << std::endl;
    }
    ...
    void submit_order(Order* order)
    {
      ...
      auto submitted = std::make_shared<OrderSubmitted>(std::make_shared<Order>(*order), clock_->now());
      msgbus_->send(Endpoints::EXEC_ENGINE_PROCESS, submitted);

      // If not connected, buffer the order for later
      if (!connected_)
      {
        std::lock_guard<std::mutex> lock(pending_mutex_);
        pending_orders_.push_back(std::make_shared<Order>(*order));
        std::cout << "[BinanceExecutionClient] Buffered order " << order->order_id() << " (disconnected)" << std::endl;
        return;
      }

      submit_order_internal(order);
    }

    void submit_order_internal(Order* order)
    {
      ...
      params << "symbol=" << order->instrument_id() << "&side=" << side_to_binance(order->side())
             << "&type=" << type_to_binance(order->type()) << "&quantity=" << std::fixed << std::setprecision(8)
             << order->quantity().as_double() << "&newClientOrderId=" << url_encode(order->order_id())
             << "&recvWindow=" << config_.recv_window_ms << "&timestamp=" << timestamp_to_ms(clock_->now());
      ...
      auto t = std::thread(
          [this, query, order_id = order->order_id()]()
          {
            try
            {
              // Rate Limit: Consume 1 weight before making request
              if (rate_limiter_)
              {
                rate_limiter_->consume(1.0);
              }
    ...
    void cancel_order(Order* order)
    {
      ...
      std::ostringstream params;
      params << "symbol=" << order->instrument_id() << "&origClientOrderId=" << url_encode(order->order_id())
             << "&recvWindow=" << config_.recv_window_ms << "&timestamp=" << timestamp_to_ms(clock_->now());
   ...
   private:
    ...
    void run_user_data_stream()
    {
      while (running_)
      {
        try
        {
          ...
          // Flush any buffered orders now that we're connected
        ...
        catch (const std::exception& e)
        {
          ...
          // SAFETY: Attempt emergency cancel of all open orders on disconnect
          emergency_cancel_open_orders();
    ...
    void emergency_cancel_open_orders()
    {
      std::cout << "[BinanceExecutionClient] SAFETY: Attempting emergency cancel of all open orders..." << std::endl;
      ...
      {
        cancel_order(order.get());
      }
    }

    void flush_pending_buffers()
    {
      std::vector<std::shared_ptr<Order>> to_send;
      {
        std::lock_guard<std::mutex> lock(pending_mutex_);
        ...
        std::cout << "[BinanceExecutionClient] Flushing " << pending_orders_.size() << " buffered orders" << std::endl;
        to_send = std::move(pending_orders_);
        pending_orders_.clear();
      }

      for (auto& order : to_send)
      {
        submit_order_internal(order.get());
      }
    }
    ...
    void process_execution_report(const json::object& data)
    {
      ...
      else if (exec_type == "TRADE")
      {
        ...
        Fill fill(order->order_id(), order->instrument_id(), Price(last_price), Quantity(last_qty), order->side(), ts);
      ...
      try
      {
        ...
        if (obj.contains("status"))
        {
          ...
          if (status == "FILLED")
          {
            ...
            Fill fill(order->order_id(), order->instrument_id(), Price(avg_price), Quantity(exec_qty), order->side(), ts);
    ...
    void process_cancel_response(const OrderId& order_id, const std::string& response)
    {
      try
      {
        ...
        if (obj.contains("code") && obj.contains("msg"))
        {
          std::cerr << "[BinanceExecutionClient] Cancel failed: " << std::string(obj["msg"].as_string()) << std::endl;
          return;
        }
    ...
    // Pending order buffer (for orders submitted during disconnect)
  ...
  BinanceExecutionClient::BinanceExecutionClient(
      const ClientId& client_id,
      const AccountId& account_id,
      MessageBus* msgbus,
      Clock* clock,
  ...
  void BinanceExecutionClient::sync_open_orders()
  {
    impl_->sync_open_orders();
  }
