...
namespace npcTrading
{
  ...
  DataEngine::DataEngine(MessageBus* msgbus, Cache* cache, Clock* clock, const DataEngineConfig& config)
      : Component("DataEngine", msgbus, cache, clock),
        config_(config)
  {
  }
  ...
  void DataEngine::on_initialize()
  {
    ...
    msgbus_->register_handler(
        Endpoints::DATA_ENGINE_EXECUTE, [this](const std::shared_ptr<Message>& msg) { handle_execute(msg); });

    msgbus_->register_handler(
        Endpoints::DATA_ENGINE_PROCESS, [this](const std::shared_ptr<Message>& msg) { handle_process(msg); });

    // Alias for historical data injection - same handler as process
    msgbus_->register_handler(
        Endpoints::DATA_ENGINE_PROCESS_HISTORICAL, [this](const std::shared_ptr<Message>& msg) { handle_process(msg); });

    msgbus_->register_request_handler(
        Endpoints::DATA_ENGINE_REQUEST, [this](const std::shared_ptr<Request>& req) { return handle_request(req); });

    msgbus_->register_handler(
        Endpoints::DATA_ENGINE_RESPONSE, [this](const std::shared_ptr<Message>& msg) { handle_response(msg); });
  }
  ...
  ClientId DataEngine::select_client(const InstrumentId& instrument_id, const ClientId& requested_client_id) const
  {
  ...
  void DataEngine::handle_execute(const std::shared_ptr<Message>& msg)
  {
    ...
    if (auto sub = std::dynamic_pointer_cast<SubscribeQuotes>(msg))
    {
      ...
      else
      {
        ...
        {
          log_warning(
              "SubscribeQuotes: " + instrument_id + " already subscribed via " + pinned + ", ignoring requested client " +
              sub->client_id());
        }
      }
      return;
    ...
    if (auto sub = std::dynamic_pointer_cast<SubscribeTrades>(msg))
    {
      ...
      else
      {
        ...
        {
          log_warning(
              "SubscribeTrades: " + instrument_id + " already subscribed via " + pinned + ", ignoring requested client " +
              sub->client_id());
        }
      }
      return;
    ...
    if (auto sub = std::dynamic_pointer_cast<SubscribeOrderBook>(msg))
    {
      ...
      if (refcount == 1)
      {
        ...
        if (auto* client = get_client(chosen))
        {
          ...
          log_debug(
              "Subscribed order book for " + instrument_id + " depth=" + std::to_string(active_depth) + " via " + chosen);
        }
      }
      else
      {
        ...
        if (new_max_depth > active_depth)
        {
          ...
          if (client_it != book_client_.end())
          {
            if (auto* client = get_client(client_it->second))
            {
              ...
              client->subscribe_order_book(instrument_id, active_depth);
              log_debug(
                  "Resubscribed order book for " + instrument_id + " depth " + std::to_string(old_depth) + " -> " +
        ...
        {
          log_warning(
              "SubscribeOrderBook: " + instrument_id + " already subscribed via " + pinned + ", ignoring requested client " +
              sub->client_id());
        }
      }
      return;
    ...
    if (auto sub = std::dynamic_pointer_cast<SubscribeBars>(msg))
    {
      ...
      else
      {
        ...
        {
          log_warning(
              "SubscribeBars: " + key + " already subscribed via " + pinned + ", ignoring requested client " +
              sub->client_id());
        }
      }
      return;
  ...
  void DataEngine::handle_process(const std::shared_ptr<Message>& msg)
  {
    ...
    if (auto quote_msg = std::dynamic_pointer_cast<QuoteTickMessage>(msg))
    {
      ...
      monitoring::MetricsCollector::instance().increment_quotes_received();
    ...
    if (auto trade_msg = std::dynamic_pointer_cast<TradeTickMessage>(msg))
    {
      ...
      monitoring::MetricsCollector::instance().increment_trades_received();
    ...
    if (auto book_msg = std::dynamic_pointer_cast<OrderBookMessage>(msg))
    {
      ...
      monitoring::MetricsCollector::instance().increment_order_book_updates();
    ...
    if (auto bar_msg = std::dynamic_pointer_cast<BarMessage>(msg))
    {
      ...
      cache_->add_bar(bar);
      monitoring::MetricsCollector::instance().increment_bars_received();

      // Topic uses same key format as bar_key() for consistency
      std::string topic = "MarketData.Bar." + bar.bar_type().instrument_id() + "|" + bar.bar_type().spec();
      msgbus_->publish(topic, msg);
      return;
    }
  ...
  std::shared_ptr<Response> DataEngine::handle_request(const std::shared_ptr<Request>& req)
  {
    ...
    if (auto instr_req = std::dynamic_pointer_cast<RequestInstrument>(req))
    {
      ...
      if (client == nullptr)
      {
        ...
        return std::make_shared<InstrumentResponse>(std::nullopt, "No client available for venue: " + venue);
      }
    ...
    if (auto bars_req = std::dynamic_pointer_cast<RequestBarsRange>(req))
    {
      ...
      if (client == nullptr)
      {
        ...
        return std::make_shared<BarsResponse>(std::vector<Bar>{}, "No client available for instrument: " + instrument_id);
      }
      ...
      log_debug("Received and published " + std::to_string(bars.size()) + " bars for " + bar_key(bar_type));
  ...
  LiveDataEngine::LiveDataEngine(MessageBus* msgbus, Cache* cache, Clock* clock, const LiveDataEngineConfig& config)
      : DataEngine(msgbus, cache, clock, config),
        live_config_(config)
  {
  }
